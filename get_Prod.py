...
class getPro(MagicSetup):
    def __init__(
        ...
        if self.term == "bgradu" or self.term == "curluxb" or self.term == "ugradu":
            ...
            if self.comp == "r" or self.comp == "tot" or self.comp == "all":
                ...
                self.durdr = rderavg(
                    self.gr.vr, eta=self.gr.radratio, spectral=True, exclude=False
                )
                self.durdt = (1.0 / r3D) * thetaderavg(self.gr.vr, order=4)
                self.durdp = (1.0 / (r3D * sin(th3D))) * phideravg(self.gr.vr, order=4)
            ...
            if self.comp == "theta" or self.comp == "tot" or self.comp == "all":
                ...
                self.dutdr = rderavg(
                    self.gr.vtheta, eta=self.gr.radratio, spectral=True, exclude=False
                )
                self.dutdt = (1.0 / r3D) * thetaderavg(self.gr.vtheta, order=4)
                self.dutdp = (1.0 / (r3D * sin(th3D))) * phideravg(
                    self.gr.vtheta, order=4
                )
            ...
            if self.comp == "phi" or self.comp == "tot" or self.comp == "all":
                ...
                self.dupdr = rderavg(
                    self.gr.vphi, eta=self.gr.radratio, spectral=True, exclude=False
                )
                self.dupdt = (1.0 / r3D) * thetaderavg(self.gr.vphi, order=4)
                self.dupdp = (1.0 / (r3D * sin(th3D))) * phideravg(
                    self.gr.vphi, order=4
                )
        ...
        if self.term == "ugradb" or self.term == "curluxb":
            ...
            if self.comp == "r" or self.comp == "all":
                ...
                self.dbrdr = rderavg(
                    self.gr.Br, eta=self.gr.radratio, spectral=True, exclude=False
                )
                self.dbrdt = (1.0 / r3D) * thetaderavg(self.gr.Br, order=4)
                self.dbrdp = (1.0 / (r3D * sin(th3D))) * phideravg(self.gr.Br, order=4)
            ...
            if self.comp == "theta" or self.comp == "all":
                ...
                self.dbtdr = rderavg(
                    self.gr.Btheta, eta=self.gr.radratio, spectral=True, exclude=False
                )
                self.dbtdt = (1.0 / r3D) * thetaderavg(self.gr.Btheta, order=4)
                self.dbtdp = (1.0 / (r3D * sin(th3D))) * phideravg(
                    self.gr.Btheta, order=4
                )
            ...
            if self.comp == "phi" or self.comp == "all":
                ...
                self.dbpdr = rderavg(
                    self.gr.Bphi, eta=self.gr.radratio, spectral=True, exclude=False
                )
                self.dbpdt = (1.0 / r3D) * thetaderavg(self.gr.Bphi, order=4)
                self.dbpdp = (1.0 / (r3D * sin(th3D))) * phideravg(
                    self.gr.Bphi, order=4
                )
        ...
        if self.term == "bgradu":
            ...
            if self.comp == "r" or self.comp == "all":
                self.bgradu_r = (
                    ...
                    - (
                        (self.gr.Btheta * self.gr.vtheta + self.gr.Bphi * self.gr.vphi)
                        / r3D
                    )
                )
            ...
            if self.comp == "theta" or self.comp == "all":
                self.bgradu_t = (
                    ...
                    - ((self.gr.Bphi * self.gr.vphi) / (r3D * tan(th3D)))
                )
            ...
            if self.comp == "phi" or self.comp == "all":
                self.bgradu_p = (
                    ...
                    + (self.gr.Bphi * self.gr.vtheta) / (r3D * tan(th3D))
                )
        ...
        if self.term == "ugradb":
            ...
            if self.comp == "r" or self.comp == "all":
                self.ugradb_r = (
                    ...
                    - (
                        (self.gr.vtheta * self.gr.Btheta + self.gr.vphi * self.gr.Bphi)
                        / r3D
                    )
                )
            ...
            if self.comp == "theta" or self.comp == "all":
                self.ugradb_t = (
                    ...
                    - ((self.gr.vphi * self.gr.Bphi) / (r3D * tan(th3D)))
                )
            ...
            if self.comp == "phi" or self.comp == "all":
                self.ugradb_p = (
                    ...
                    + (self.gr.vphi * self.gr.Btheta) / (r3D * tan(th3D))
                )
        ...
        if self.term == "ugradu":
            ...
            if self.comp == "theta" or self.comp == "all":
                self.ugradu_t = (
                    ...
                    - ((self.gr.vphi * self.gr.vphi) / (r3D * tan(th3D)))
                )
            ...
            if self.comp == "phi" or self.comp == "all":
                self.ugradu_p = (
                    ...
                    + (self.gr.vphi * self.gr.vtheta) / (r3D * tan(th3D))
                )
        ...
        if self.term == "Coriolis":
            ...
            if self.comp == "phi" or self.comp == "all":
                self.cor_p = -fac * (
                    self.gr.vtheta * cos(th3D) + self.gr.vr * sin(th3D)
                )
                self.dat = self.cor_p
            if self.comp == "tot":
                self.cor_r = fac * self.gr.vphi * sin(th3D)
        ...
        if self.term == "pressure":
            ...
            if self.comp == "r" or self.comp == "all":
                self.dpdr = -rderavg(
                    self.gr.pre, eta=self.gr.radratio, spectral=True, exclude=False
                )
            if self.comp == "theta" or self.comp == "all":
                self.dpdt = -(1.0 / r3D) * thetaderavg(self.gr.pre, order=4)
            if self.comp == "phi" or self.comp == "all":
                self.dpdp = -(1.0 / (r3D * sin(th3D))) * phideravg(self.gr.pre, order=4)
    ...
    def slice(
        ...
        plot(
            self.gr.radius[0] * sin(self.gr.colatitude),
            self.gr.radius[0] * cos(self.gr.colatitude),
            "k-",
        )
        plot(
            self.gr.radius[-1] * sin(self.gr.colatitude),
            self.gr.radius[-1] * cos(self.gr.colatitude),
            "k-",
        )
