...
def getRms(u, r, th, th2D, phi):
    ...
    if u.ndim == 3:
        vol = 4.0 / 3.0 * pi * (r.max() ** 3 - r.min() ** 3)
...
def getVort(vr, vt, vp, r, th, phi):
    ...
    if vr.ndim == 3:
        ...
        curl_r = 1.0 / (r3D * sin(th3D)) * (thetaderavg(vp * sin(th3D)) - phideravg(vt))
        curl_t = (
            1.0 / r3D * (1.0 / (sin(th3D)) * phideravg(vr) - rderavg(r3D * vp, eta=eta))
        )
        curl_p = 1.0 / r3D * (rderavg(r3D * vt, eta=eta) - thetaderavg(vr))
        ...
        curl_r = 1.0 / (r2D * sin(th2D)) * (thetaderavg(vp * sin(th2D)))
        curl_t = 1.0 / r2D * (-rderavg(r2D * vp, eta=eta))
        curl_p = 1.0 / r2D * (rderavg(r2D * vt, eta=eta) - thetaderavg(vr))
